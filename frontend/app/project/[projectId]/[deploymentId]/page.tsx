...
export default function DeploymentDetailsPage({ params }: { params?: { projectId: string, deploymentId: string } }) {
  ...
  useEffect(() => {
    if (params?.projectId && params?.deploymentId) {
      ...
      setDeploymentId(pathParts[pathParts.length - 1]);
      setProjectId(pathParts[pathParts.length - 2]);
    }
  }, [params]);
